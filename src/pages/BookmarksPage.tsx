...
import { getBookmarks, removeBookmark, type BookmarkedPost } from '@/lib/bookmarkStore';
...
export function BookmarksPage() {
  ...
  const displayed = activeTab === 'All' ? bookmarks : bookmarks;
    ...
    <div>
      ...
      <div className="flex border-b border-white/10">
        {tabs.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab}
            {activeTab === tab && (
              <motion.div layoutId="bookmarkTab"
                className="absolute bottom-0 left-1/2 -translate-x-1/2 w-10 h-0.5 bg-[#ef4444] rounded-full" />
            )}
          </button>
        ))}
      </div>
      ...
      {displayed.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
          ...
          <p className="text-sm text-[#71767b] mt-1 max-w-xs">
      ...
      <AnimatePresence>
        {displayed.map((item, i) => (
          <motion.div key={item.id}
            ...
            {/* Avatar */}
            <div className="shrink-0 cursor-pointer" onClick={e => { e.stopPropagation(); navigate(`/user/${item.userId}`); }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1 mb-0.5">
                ...
                <span className="text-[#71767b] text-xs truncate">{item.userHandle}</span>
                <span className="text-[#71767b] text-xs">· {item.timestamp}</span>
              </div>
              <p className="text-sm text-[#e7e9ea] leading-relaxed mb-2 line-clamp-3">{item.content}</p>
            ...
            <button onClick={e => handleRemove(e, item.id)}
              className="p-2 rounded-full hover:bg-red-500/10 hover:text-[#ef4444] text-[#71767b] transition-colors shrink-0 self-start mt-1">
