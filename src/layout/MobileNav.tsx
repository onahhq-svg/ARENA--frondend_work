...
import { Home, Search, Bell, Mail, Zap, Trophy, Target, TrendingUp, Settings, X, Radio, Users, Crown } from 'lucide-react';
...
const bottomNavItems = [
  ...
  { icon: Crown,  label: 'Tipster',     href: '/predictions' },
];
...
export function MobileNav() {
  ...
  const navItems = bottomNavItems.map(item =>
    item.label === 'Tipster'
      ? { ...item, href: isTipster ? '/tipster/u1' : '/predictions', label: isTipster ? 'Tipster' : 'Predict' }
      : item
  );
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
              ...
              <nav className="flex-1 overflow-y-auto py-3">
                {menuItems.map(item => {
                  ...
                  return (
                    <button key={item.label} onClick={() => { navigate(item.href); setMenuOpen(false); }}
              ...
              <div className="px-5 py-4 border-t border-white/5 flex gap-3">
                <button onClick={() => { navigate('/wallet'); setMenuOpen(false); }}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-black/95 backdrop-blur-lg border-t border-[#ef4444]/30 z-40 md:hidden">
        <div className="flex items-center justify-around py-2">
          {navItems.map(item => {
            ...
            return (
              <motion.button key={item.label} onClick={() => navigate(item.href)}
