...
import { Settings, ChevronRight, Mail, Bell } from 'lucide-react';
    ...
    <header className={cn(
      ...
      <div className="border-b border-[#2f3336]">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          {/* Right — Messages + Notifications (mobile always) + Settings (when available) */}
          <div className="flex items-center gap-0.5">
            ...
            <button onClick={() => navigate('/messages')}
              ...
              <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#ef4444] text-white text-[9px] font-bold rounded-full flex items-center justify-center">2</span>
            </button>
            ...
            <button onClick={() => navigate('/notifications')}
              ...
              <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-[#ef4444] text-white text-[9px] font-bold rounded-full flex items-center justify-center">9+</span>
            </button>

            {/* Settings dropdown — when settingsOptions provided */}
                            ...
                            return (
                              <div key={option.label}
                                ...
                                <div>
                                  ...
                                  {option.description && <p className="text-xs text-[#71767b]">{option.description}</p>}
                                </div>
                                <div className={cn("w-10 h-5 rounded-full transition-colors relative flex-shrink-0",
        ...
        {!title && (
          <div className="flex relative">
            {tabs.map((tab) => (
              <button key={tab}
                className={cn("flex-1 py-3 text-center font-medium transition-colors relative",
